@if ($config['raw_only'] === false)
  @if($metar)
    <div class="card mb-2">
      <div class="card-header p-1"><h5 class="m-1 p-0">Current Weather At {{ strtoupper($config['icao']) }}<i class="fas fa-cloud-sun float-right"></i></h5></div>
...
@endif
<div class="card mb-2">
  <div class="card-header p-1"><h5 class="m-1 p-0">Latest WX Reports For {{ strtoupper($config['icao']) }}<i class="fas fa-cloud-sun float-right"></i></h5></div>
  <div class="card-body p-0">
    <table class="table table-sm table-striped table-borderless mb-0">
      <tr>
        ...
        <tr>
          <th scope="row">TAF</th>
          <td>@if(isset($taf)) {{ $taf['raw'] }} @else @lang('widgets.weather.nometar') @endif</td>
        </tr>
    </table>
  </div>
</div>
