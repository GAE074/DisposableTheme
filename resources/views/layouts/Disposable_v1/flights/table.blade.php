      ...
      <table class="table table-sm table-borderless table-striped text-center mb-1">
        <tr>
          <th class="text-left" style="width: 50%;">
            <a href="{{route('frontend.airports.show', ['id' => $flight->dpt_airport_id])}}">{{ optional($flight->dpt_airport)->name ?? $flight->dpt_airport_id }} / {{$flight->dpt_airport_id}}</a>
          </th>
          <th class="text-right" style="width: 50%;">
            <a href="{{route('frontend.airports.show', ['id' => $flight->arr_airport_id])}}">{{ optional($flight->arr_airport)->name ?? $flight->arr_airport_id }} / {{$flight->arr_airport_id}}</a>
          </th>
        </tr>
        ...
        <table class="table table-sm table-striped table-borderless mb-0">
            @if($flight->alt_airport_id)
              <tr>
                ...
                <td>
                  <a href="{{route('frontend.airports.show', ['id' => $flight->alt_airport_id])}}">
                    {{ optional($flight->alt_airport)->name ?? $flight->alt_airport_id }} / {{$flight->alt_airport_id}}
                  </a>
                </td>
              </tr>
            @endif
